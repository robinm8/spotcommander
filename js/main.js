...
function setGlobalVariables(global_variables)
{
	...
	if(shc(ua, 'Firefox') && shc(ua, 'Mobile') || shc(ua, 'Firefox') && shc(ua, 'Tablet') || shc(ua, 'Presto') || shc(ua, 'Ubuntu') && shc(ua, 'Mobile') || shc(ua, 'Ubuntu') && shc(ua, 'Tablet') || shc(ua, 'Windows Phone'))
	{
		ua_is_supported = false;
	}
	else if(ua_is_android)
	{
		if(shc(ua, 'Android 2.3') || shc(ua, 'Android 4') || shc(ua, 'Android 5'))
		{
	...
	{
		if(shc(ua, 'OS 5_') || shc(ua, 'OS 6_') || shc(ua, 'OS 7_') || shc(ua, 'OS 8_'))
		{
			if(ua_is_webkit) ua_is_supported = true;
		}
	}
	...
	$.get('main.php?global_variables', function(xhr_data)
	{
		...
		$(document).on(pointer_event, 'div.actions_div, span.actions_span', function(event)
		{
			...
			for(var i = 0; i < actions.length; i++)
			{
				...
				{
					deleteUrisFromPlaylist(data.uri, data.uris, data.positions, data.snapshot_id, data.div);
				}
				...
				else if(action == 'show_edit_playlist_activity')
				{
					showEditPlaylistActivity(data.uri, data.isauthorizedwithspotify);
				}
				else if(action == 'remove_playlist')
				{
					removePlaylist(data.id);
				}
				else if(action == 'clear_recent_playlists')
				{
