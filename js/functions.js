	...
	xhr_nowplaying = $.get('nowplaying.php', function(xhr_data)
	{
		...
		if(type == 'manual' || nowplaying_last_data != xhr_data)
		{
			...
			if(type == 'manual' || nowplaying_last_uri != nowplaying.uri)
			{
				...
				if(settings_update_lyrics && isActivity('lyrics', '')) replaceActivity('lyrics', '', 'artist='+encodeURIComponent(nowplaying.artist)+'&title='+encodeURIComponent(nowplaying.title));
...
function deleteUrisFromPlaylist(uri, uris, positions)
{
	$.post('playlists.php?delete_uris_from_playlist&'+getCurrentTime(), { uri: uri, uris: uris, positions: positions }, function(xhr_data)
	{
		if(xhr_data == 'error')
		{
			showToast('Could not delete track from playlist', 4);
		}
		else
		{
			showToast('Track deleted from playlist', 4);
			refreshActivity();
		}
	});
}
