...
{
	echo delete_uris_from_playlists($_POST['uri'], $_POST['uris'], intval($_POST['positions']));
}
...
elseif(isset($_GET['browse']))
{
	...
	else
	{
		...
		else
		{
			...
			$actions_dialog['actions'][] = array('text' => 'Queue tracks', 'keys' => array('actions', 'uris', 'randomly'), 'values' => array('hide_dialog queue_uris', $playlist_uri, 'false'));
			$actions_dialog['actions'][] = array('text' => 'Queue tracks randomly', 'keys' => array('actions', 'uris', 'randomly'), 'values' => array('hide_dialog queue_uris', $playlist_uri, 'true'));
			...
			$activity['actions'][] = array('action' => array('Shuffle play', ''), 'keys' => array('actions', 'uri'), 'values' => array('shuffle_play_uri', $playlist_uri));
			$activity['actions'][] = array('action' => array('Share', ''), 'keys' => array('actions', 'title', 'uri'), 'values' => array('share_uri', ucfirst(hsc($name)), rawurlencode(uri_to_url($playlist_uri))));
			...
			if(!$is_starred && !playlist_is_saved($playlist_uri)) echo '<div class="green_button_div green_button_below_cover_art_div actions_div" data-actions="import_playlist" data-uri="' . $playlist_uri . '" data-highlightclass="light_green_highlight" onclick="void(0)">Add to my playlists</div>';
			...
			foreach($tracks as $track)
			{
				...
				$actions_dialog['actions'][] = array('text' => 'Delete from playlist', 'keys' => array('actions', 'uri', 'uris', 'positions'), 'values' => array('hide_dialog delete_uris_from_playlist', $playlist_uri, $uri, $position));
