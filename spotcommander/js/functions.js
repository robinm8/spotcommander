...
function showMenu()
{
	...
	if(activity_has_cover_art_opacity) $('div#top_actionbar_inner_div').removeClass('top_actionbar_inner_opacity_div shadow_down_black_56_img_div').css('background-color', '');
...
function hideMenu()
{
	...
	if(activity_has_cover_art_opacity && !isDisplayed('div#top_actionbar_shadow_div')) $('div#top_actionbar_inner_div').addClass('shadow_down_black_56_img_div').css('background-color', 'rgba(104, 159, 56, '+activity_cover_art_opacity+')');
}
...
function refreshNowplaying(type)
{
	...
	xhr_nowplaying = $.get('nowplaying.php', function(xhr_data)
	{
		...
		if(type == 'manual' || nowplaying_last_data != xhr_data)
		{
			...
			$('div#bottom_actionbar_inner_left_div > div > div > div').removeClass('play_white_24_img_div pause_white_24_img_div').addClass(nowplaying.play_pause+'_white_24_img_div');
...
function setCoverArtOpacity()
{
	...
	if(scroll_position < height)
	{
		...
		$('div#top_actionbar_shadow_div').hide();
		$('div#top_actionbar_inner_div').addClass('shadow_down_black_56_img_div').removeClass('top_actionbar_inner_opacity_div').css('background-color', 'rgba(104, 159, 56, '+activity_cover_art_opacity+')');
	}
	else
	{
		$('div#top_actionbar_shadow_div').show();
		$('div#top_actionbar_inner_div').removeClass('shadow_down_black_56_img_div top_actionbar_inner_opacity_div').css('background-color', '');
	}
}
...
function importPlaylists(uris)
{
	...
	else
	{
		...
		$.post('playlists.php?import_playlists&'+getCurrentTime(), { uris: uris }, function(xhr_data)
		{
			if(xhr_data == 'error')
			{
				showDialog({ title: 'Import Playlists', body_class: 'dialog_message_div', body_content: 'Could not import one or more playlists. Try again.', button1: null, button2: null, cookie: null });
			}
			else
			{
				if(div.length)
...
function checkForDialogs()
{
	...
	else if(ua_is_os_x && !ua_is_standalone)
	{
		...
		if(!isCookie(cookie.id)) showDialog({ title: 'OS X Tip', body_class: 'dialog_message_div', body_content: 'Install Fluid to run '+project_name+' as a standalone app.', button1: { text: 'LATER', keys : ['actions'], values: ['hide_dialog'] }, button2: { text: 'HELP', keys : ['actions', 'uri'], values: ['open_external_activity', project_website+'?os_x_integration'] }, cookie: cookie });
	}
	else if(ua_is_msie && ua_is_pinnable_msie && !window.external.msIsSiteMode())
	{
		...
		if(!isCookie(cookie.id)) showDialog({ title: 'Windows Desktop Tip', body_class: 'dialog_message_div', body_content: 'Pin '+project_name+' to the taskbar to get additional features.', button1: { text: 'LATER', keys : ['actions'], values: ['hide_dialog'] }, button2: { text: 'HELP', keys : ['actions', 'uri'], values: ['open_external_activity', project_website+'?windows_desktop_integration'] }, cookie: cookie });
	}
}
