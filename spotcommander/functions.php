...
{
	$get_uri_append = ($size == 'small') ? '' : '&combined';
...
function get_playlists_as_json($include_starred)
{
	...
	if($include_starred) $return['Starred'] = 'spotify:user:' . get_spotify_username() . ':starred';
...
{
	$api_uri = 'https://api.spotify.com/v1/users/' . get_spotify_username() . '/playlists?limit=50';
	$api_headers = array('Authorization: Bearer ' . get_spotify_token());
...
function get_user($username)
{
	...
	$files = get_external_files(array('https://api.spotify.com/v1/users/' . $username, 'https://api.spotify.com/v1/users/' . $username . '/playlists?limit=50'), array('Authorization: Bearer ' . get_spotify_token()), null);
	$profile = json_decode($files[0], true);
	$playlists = json_decode($files[1], true);
...
function check_for_errors()
{
	...
	elseif(empty(daemon_qdbus_select()))
	{
		$code =  4;
	}
...
function check_for_updates()
{
	...
	$installed = (isset($_COOKIE['installed_' . project_version]) && is_numeric($_COOKIE['installed_' . project_version])) ? round($_COOKIE['installed_' . project_version] / 1000) : 0;

	$files = get_external_files(array(project_website . 'api/1/latest-version/?version=' . rawurlencode(number_format(project_version, 1)) . '&installed=' . rawurlencode($installed) . '&uname=' . rawurlencode($system_information['uname']) . '&ua=' . rawurlencode($system_information['ua'])), null, null);
