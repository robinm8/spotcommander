...
function follow_user($artist, $title, $uri, $username, $following)
{
	...
	$files = get_external_files(array('https://api.spotify.com/v1/me/following?type=user&ids=' . $username), array('Authorization: Bearer ' . get_spotify_token()), array($action, null));
	$response = $files[0];
