...
else
{
	...
	else
	{
		...
		foreach($tracks as $track)
		{
			...
			$icon = track_is_playing($uri, 'icon');
			$text = track_is_playing($uri, 'text');

			if($is_action)
			{
				$icon = $uri . '_24_img_div';
				$text = '';
			}
			else
			{
			...
			echo '
				...
				<div class="list_item_main_inner_icon_div"><div class="img_div img_24_div ' . $icon . '"></div></div>
				<div class="list_item_main_inner_text_div"><div class="list_item_main_inner_text_upper_div ' . $text . '">' . hsc($title) . '</div><div class="list_item_main_inner_text_lower_div">' . hsc($artist) . '</div></div>
				</div>
				</div>
			...
			if(!$is_action) echo '<div title="More" class="show_actions_dialog_div actions_div" data-actions="show_actions_dialog" data-dialogactions="' . base64_encode(json_encode($actions_dialog)) . '" data-highlightclass="dark_grey_highlight" onclick="void(0)"><div class="img_div img_24_div overflow_24_img_div"></div></div>';
