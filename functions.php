...
function get_cover_art($uri)
{
	...
	if(!$cover_art)
	{
		...
		else
		{
			...
			$cover = (empty($cover[1])) ? 'error' : 'http://o.scdn.co/640/' . $cover[1];
		}
...
function get_playlist($playlist_uri)
{
	...
	if(empty($return))
	{
		...
		$api_uri = ($is_starred) ? 'https://api.spotify.com/v1/users/' . $user . '/starred?fields=name,owner(id),public,tracks(items,total,limit)' : 'https://api.spotify.com/v1/users/' . $user . '/playlists/' . uri_to_id($playlist_uri) . '?fields=name,owner(id),public,tracks(items,total,limit)';
		...
		if(!empty($metadata['name']))
		{
			...
			foreach($tracks as $track)
			{
				if(!empty($track['track']) && $track['track']['uri'] != 'spotify:track:null')
				{
...
function delete_uris_from_playlists($uri, $uris, $positions)
{
	$user = explode(':', $uri);
	$user = $user[2];

	$json = json_encode(array('tracks' => array(array('uri' => $uris, 'positions' => array($positions)))));
...
function get_chart($chart)
{
	...
	$files = get_external_files(array('http://charts.spotify.com/api/tracks/most_' . $chart . '/' . strtolower($country) . '/weekly/latest'), null, null);
	$chart = json_decode($files[0], true);

	if(!empty($chart['tracks']))
	{
		$return = array();
		...
		foreach($tracks as $track)
		{
			$return['tracks'][$i]['artist'] = $track['artist_name'];
			$return['tracks'][$i]['title'] = $track['track_name'];
			$return['tracks'][$i]['uri'] = url_to_uri($track['track_url']);
			$return['tracks'][$i]['plays'] = $track['num_streams'];
...
function url_to_uri($uri)
{
	...
	elseif(ssw($uri, 'https://play.spotify.com/track/'))
	{
		$uri = str_replace('https://play.spotify.com/track/', '', $uri);
		$uri = 'spotify:track:' . $uri;
	}
