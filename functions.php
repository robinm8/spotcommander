...
{
	$uri = strtolower($uri);
	$icon = (get_uri_type($uri) == 'local') ? 'local_24_img_div' : 'track_24_img_div';

	if($div == 'icon')
	{
		return (!empty($_COOKIE['nowplaying_uri']) && $_COOKIE['nowplaying_uri'] == $uri) ? 'playing_24_img_div' : $icon;
	}
	elseif($div == 'text')
	{
...
function get_cover_art($uri)
{
	...
	if(!$cover_art)
	{
		...
		{
			$metadata = get_tracks(array($uri));
			$track = $metadata[0];

			$cover = (empty($track['album']['images'][0]['url'])) ? 'error' : $track['album']['images'][0]['url'];
		}
		else
		{
			...
			$cover = (empty($cover[1])) ? 'error' : $cover[1];
		}

		if($cover != 'error')
		{
			$cover = preg_replace(array('/^\/\//', '/^https?:\/\/.*?\//', '/\/thumb\//', '/\/image\//', '/\/unbranded\//'), array('http://', 'http://o.scdn.co/', '/640/', '/640/', '/640/'), $cover);
...
function queue_uris($uris, $randomly)
{
	...
	if(!empty($insert))
	{
		...
		for($f = 1; $f < count($insert); $f++)
		{
			$insert_id++;

			$exec .= "UNION SELECT " . $insert_id . ", '" . sqlite_escape($insert[$f]['artist']) . "', '" . sqlite_escape($insert[$f]['title']) . "', '" . sqlite_escape($insert[$f]['uri']) . "', '" . sqlite_escape($insert_id) . "' ";
		}
...
function get_playlist($playlist_uri)
{
	...
	if(empty($return))
	{
		...
		if(!empty($metadata['name']))
		{
			...
			$return['cover_art_uri'] = (empty($metadata['images'][0]['url'])) ? null : $metadata['images'][0]['url'];
	...
	if(!empty($metadata['items']))
	{
		...
		if(!empty($insert))
		{
			...
			for($f = 1; $f < count($insert); $f++)
			{
				...
				$exec .= "UNION SELECT " . $insert_id . ", '" . sqlite_escape($insert[$f]['name']) . "', '" . sqlite_escape($insert[$f]['uri']) . "' ";
			}
...
function clear_recent_playlists()
{
...
function import_saved_spotify_tracks()
{
	...
	if(!empty($metadata['items']))
	{
		...
		if(!empty($insert))
		{
			...
			$exec = "INSERT INTO library SELECT " . $insert_id . " AS id, 'track' AS type, '" . sqlite_escape($insert[0]['artist']) . "' AS artist, '" . sqlite_escape($insert[0]['title']) . "' AS title, '" . sqlite_escape($insert[0]['uri']) . "' AS uri ";

			for($f = 1; $f < count($insert); $f++)
			{
				$insert_id++;

				$exec .= "UNION SELECT " . $insert_id . ", 'track', '" . sqlite_escape($insert[$f]['artist']) . "', '" . sqlite_escape($insert[$f]['title']) . "', '" . sqlite_escape($insert[$f]['uri']) . "' ";
			}

			$exec = rtrim($exec);

			db_exec('library', $exec);
		}
...
function get_external_files($uris, $headers, $post)
{
	...
	{
		$uri = $uris[$i];
		$ua = (preg_match('/^https?:\/\/open\.spotify\.com\//', $uri)) ? 'Mozilla/5.0 (Android; Mobile; rv:32.0) Gecko/32.0 Firefox/32.0' : 'Mozilla/5.0 (X11; Ubuntu; Linux x86_64; rv:32.0) Gecko/20100101 Firefox/32.0';
...
function get_uri_type($uri)
{
	...
	elseif(preg_match('/^https?:\/\/o\.scdn\.co\/\w+\/\w+$/', $uri) || preg_match('/^https?:\/\/\w+\.cloudfront\.net\/\w+\/\w+$/', $uri))
	{
		$type = 'cover_art';
	}
