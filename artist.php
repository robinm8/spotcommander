...
else
{
	...
	{
		$library_action = (is_saved($artist_uri) == 'save') ? 'Save to Library' : 'Remove from Library';

		$activity['cover_art_uri'] = '';
		$activity['actions'][] = array('action' => array($library_action, ''), 'keys' => array('actions', 'artist', 'title', 'uri', 'isauthorizedwithspotify'), 'values' => array('save', rawurlencode($artist), '', $artist_uri, is_authorized_with_spotify));
		...
		echo '
			...
			<div id="cover_art_information_div"><div><div>Popularity: ' . $popularity . '</div></div><div><div>' . $albums_count . '</div></div></div>
			</div>
		...
		if(!empty($tracks))
		{
			...
			foreach($tracks as $track)
			{
				...
				echo '
					...
					<div title="Go to Album" class="actions_div" data-actions="browse_album" data-uri="' . $album_uri . '" data-highlightclass="dark_grey_highlight" onclick="void(0)"><div class="img_div img_24_div album_24_img_div"></div></div>
